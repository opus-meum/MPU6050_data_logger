...
bool MPU6050::begin(mpu6050_dps_t scale, mpu6050_range_t range, int mpua)
{
    ...
    if (fastRegister8(MPU6050_REG_WHO_AM_I) != 104)
    {
	return false;
    }
...
IntVector MPU6050::readRawAccel(void)
{
...
Vector MPU6050::readNormalizeAccel(void)
{
    ...
    na.XAxis = (float)ra.XAxis * rangePerDigit * 9.80665f;
    na.YAxis = (float)ra.YAxis * rangePerDigit * 9.80665f;
    na.ZAxis = (float)ra.ZAxis * rangePerDigit * 9.80665f;
...
IntVector MPU6050::readRawGyro(void)
{
    Wire.beginTransmission(mpuAddress);
    #if ARDUINO >= 100
